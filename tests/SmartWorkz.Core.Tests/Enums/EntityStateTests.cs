...
public class EntityStateTests
{
    ...
    #region Category Organization Tests

    [Fact]
    public void EntityState_HasLifecycleCategory()
    {
        // Assert - Lifecycle category: Active, Inactive, Archived, Deleted
        Assert.Equal(EntityState.Active, EntityState.Active);
        Assert.Equal(EntityState.Inactive, EntityState.Inactive);
        Assert.Equal(EntityState.Archived, EntityState.Archived);
        Assert.Equal(EntityState.Deleted, EntityState.Deleted);
    }

    [Fact]
    public void EntityState_HasVerificationCategory()
    {
        // Assert - Verification category: EmailVerified, EmailPending, PhoneVerified, PhonePending
        Assert.Equal(EntityState.EmailVerified, EntityState.EmailVerified);
        Assert.Equal(EntityState.EmailPending, EntityState.EmailPending);
        Assert.Equal(EntityState.PhoneVerified, EntityState.PhoneVerified);
        Assert.Equal(EntityState.PhonePending, EntityState.PhonePending);
    }

    [Fact]
    public void EntityState_HasApprovalCategory()
    {
        // Assert - Approval category: PendingApproval, Approved, Rejected
        Assert.Equal(EntityState.PendingApproval, EntityState.PendingApproval);
        Assert.Equal(EntityState.Approved, EntityState.Approved);
        Assert.Equal(EntityState.Rejected, EntityState.Rejected);
    }

    [Fact]
    public void EntityState_HasDocumentCategory()
    {
        // Assert - Document category: Draft, Submitted, InReview, Completed
        Assert.Equal(EntityState.Draft, EntityState.Draft);
        Assert.Equal(EntityState.Submitted, EntityState.Submitted);
        Assert.Equal(EntityState.InReview, EntityState.InReview);
        Assert.Equal(EntityState.Completed, EntityState.Completed);
    }

    [Fact]
    public void EntityState_HasSecurityCategory()
    {
        // Assert - Security category: PasswordReset, PasswordResetting, Locked, Suspended
        Assert.Equal(EntityState.PasswordReset, EntityState.PasswordReset);
        Assert.Equal(EntityState.PasswordResetting, EntityState.PasswordResetting);
        Assert.Equal(EntityState.Locked, EntityState.Locked);
        Assert.Equal(EntityState.Suspended, EntityState.Suspended);
    }

    [Fact]
    public void EntityState_HasPaymentCategory()
    {
        // Assert - Payment category: PaymentPending, PaymentReceived, PaymentFailed
        Assert.Equal(EntityState.PaymentPending, EntityState.PaymentPending);
        Assert.Equal(EntityState.PaymentReceived, EntityState.PaymentReceived);
        Assert.Equal(EntityState.PaymentFailed, EntityState.PaymentFailed);
    }

    [Fact]
    public void EntityState_HasOrderCategory()
    {
        // Assert - Order category: OrderPlaced, OrderConfirmed, Shipped, Delivered, Returned, Cancelled
        Assert.Equal(EntityState.OrderPlaced, EntityState.OrderPlaced);
        Assert.Equal(EntityState.OrderConfirmed, EntityState.OrderConfirmed);
        Assert.Equal(EntityState.Shipped, EntityState.Shipped);
        Assert.Equal(EntityState.Delivered, EntityState.Delivered);
        Assert.Equal(EntityState.Returned, EntityState.Returned);
        Assert.Equal(EntityState.Cancelled, EntityState.Cancelled);
    }

    [Fact]
    public void EntityState_HasInventoryCategory()
    {
        // Assert - Inventory category: OutOfStock, LowStock, Restocking, BackOrder
        Assert.Equal(EntityState.OutOfStock, EntityState.OutOfStock);
        Assert.Equal(EntityState.LowStock, EntityState.LowStock);
        Assert.Equal(EntityState.Restocking, EntityState.Restocking);
        Assert.Equal(EntityState.BackOrder, EntityState.BackOrder);
    }

    [Fact]
    public void EntityState_HasCommunicationCategory()
    {
        // Assert - Communication category: Read, Unread, Forwarded, Replied
        Assert.Equal(EntityState.Read, EntityState.Read);
        Assert.Equal(EntityState.Unread, EntityState.Unread);
        Assert.Equal(EntityState.Forwarded, EntityState.Forwarded);
        Assert.Equal(EntityState.Replied, EntityState.Replied);
    }

    [Fact]
    public void EntityState_HasSubscriptionCategory()
    {
        // Assert - Subscription category: Trial, SubscriptionActive, Paused, SubscriptionCancelled, Expired
        Assert.Equal(EntityState.Trial, EntityState.Trial);
        Assert.Equal(EntityState.SubscriptionActive, EntityState.SubscriptionActive);
        Assert.Equal(EntityState.Paused, EntityState.Paused);
        Assert.Equal(EntityState.SubscriptionCancelled, EntityState.SubscriptionCancelled);
        Assert.Equal(EntityState.Expired, EntityState.Expired);
    }

    [Fact]
    public void EntityState_HasUserAccountCategory()
    {
        // Assert - User Account category: EmailVerificationPending, PhoneVerificationPending, KYCPending, KYCApproved, KYCRejected, AccountSuspended
        Assert.Equal(EntityState.EmailVerificationPending, EntityState.EmailVerificationPending);
        Assert.Equal(EntityState.PhoneVerificationPending, EntityState.PhoneVerificationPending);
        Assert.Equal(EntityState.KYCPending, EntityState.KYCPending);
        Assert.Equal(EntityState.KYCApproved, EntityState.KYCApproved);
        Assert.Equal(EntityState.KYCRejected, EntityState.KYCRejected);
        Assert.Equal(EntityState.AccountSuspended, EntityState.AccountSuspended);
    }

    [Fact]
    public void EntityState_HasReturnCategory()
    {
        // Assert - Return/Refund category: ReturnRequested, ReturnApproved, ReturnRejected, ReturnInProgress, ReturnCompleted
        Assert.Equal(EntityState.ReturnRequested, EntityState.ReturnRequested);
        Assert.Equal(EntityState.ReturnApproved, EntityState.ReturnApproved);
        Assert.Equal(EntityState.ReturnRejected, EntityState.ReturnRejected);
        Assert.Equal(EntityState.ReturnInProgress, EntityState.ReturnInProgress);
        Assert.Equal(EntityState.ReturnCompleted, EntityState.ReturnCompleted);
    }

    [Fact]
    public void EntityState_HasTaskCategory()
    {
        // Assert - Task/Job category: Assigned, InProgress, OnHold, TaskCompleted, Failed
        Assert.Equal(EntityState.Assigned, EntityState.Assigned);
        Assert.Equal(EntityState.InProgress, EntityState.InProgress);
        Assert.Equal(EntityState.OnHold, EntityState.OnHold);
        Assert.Equal(EntityState.TaskCompleted, EntityState.TaskCompleted);
        Assert.Equal(EntityState.Failed, EntityState.Failed);
    }

    [Fact]
    public void EntityState_HasReviewCategory()
    {
        // Assert - Review/Rating category: PendingReview, UnderReview, ReviewApproved, ReviewRejected
        Assert.Equal(EntityState.PendingReview, EntityState.PendingReview);
        Assert.Equal(EntityState.UnderReview, EntityState.UnderReview);
        Assert.Equal(EntityState.ReviewApproved, EntityState.ReviewApproved);
        Assert.Equal(EntityState.ReviewRejected, EntityState.ReviewRejected);
    }

    [Fact]
    public void EntityState_HasShippingCategory()
    {
        // Assert - Shipping Status category: InTransit, OutForDelivery, DeliveredShipping
        Assert.Equal(EntityState.InTransit, EntityState.InTransit);
        Assert.Equal(EntityState.OutForDelivery, EntityState.OutForDelivery);
        Assert.Equal(EntityState.DeliveredShipping, EntityState.DeliveredShipping);
    }
