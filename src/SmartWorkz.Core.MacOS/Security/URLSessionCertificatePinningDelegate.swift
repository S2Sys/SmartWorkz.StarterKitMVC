...
/// using SPKI (Subject Public Key Info) pins for macOS applications.
///
...
class URLSessionCertificatePinningDelegate: NSObject, URLSessionDelegate {
    private let queue = DispatchQueue(label: "com.smartworkz.certificate.pinning", attributes: .concurrent)
