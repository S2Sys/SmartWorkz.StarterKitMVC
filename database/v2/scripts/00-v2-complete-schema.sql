-- ============================================
-- SmartWorkz v2 Complete Schema - Optimized
-- Purpose: Complete database setup with all V1 tables + new Master.Lookup (HierarchyID) + merged Auth.AuthTokens
-- Removed: Currencies, Languages, TimeZones (stored in Master.Lookup)
-- Removed: PasswordResetTokens, EmailVerificationTokens, TwoFactorTokens, RefreshTokens (merged to Auth.AuthTokens)
...
-- 2. Master.Countries (KEEP - geographic reference data)
...
-- 6. Master.Categories (HierarchyID)
...
-- 7. Master.MenuItems (HierarchyID)
...
-- 8. Master.GeoHierarchy (HierarchyID)
...
-- 12. REMOVED: Master.TenantUsers (DUPLICATE - using Auth.TenantUsers instead)

-- 13. Master.CacheEntries
...
-- 15. Master.ContentTemplateSections
...
-- 16. Master.TemplatePlaceholders
...
-- 17. Master.Lookup (NEW - HierarchyID for all lookups: Currencies, Languages, TimeZones)
...
-- 20. Shared.Translations (References Master.Lookup for Languages)
...
-- 25. Transaction.TransactionLog (References Master.Lookup for Currencies)
...
-- 36. Auth.AuthTokens (NEW MERGED - replaces PasswordResetTokens, EmailVerificationTokens, TwoFactorTokens, RefreshTokens)
...
CREATE TABLE Auth.AuthTokens (
    ...
    TokenType NVARCHAR(50) NOT NULL, -- 'PasswordReset', 'EmailVerification', 'TwoFactor', 'RefreshToken'
    TokenSubType NVARCHAR(50), -- 'Email', 'Authenticator', 'SMS' for TwoFactor
...
-- COMPLETION
-- ============================================
PRINT '✓ v2 Complete Schema Created Successfully';
PRINT '✓ 6 Schemas: Master, Shared, Transaction, Report, Auth';
PRINT '✓ 41 Tables Total (optimized from 48 in V1)';
PRINT '✓ CRITICAL FIXES APPLIED:';
PRINT '  ✓ Removed Master.TenantUsers (duplicate - using Auth.TenantUsers)';
PRINT '  ✓ Shared.Translations.LanguageLookupId → Master.Lookup(Id)';
PRINT '  ✓ Transaction.TransactionLog.CurrencyLookupId → Master.Lookup(Id)';
PRINT '✓ Master.Lookup (HierarchyID) for consolidated lookups (Currencies, Languages, TimeZones)';
PRINT '✓ Auth.AuthTokens (merged: PasswordReset, EmailVerification, TwoFactor, RefreshToken)';
PRINT '✓ Removed: Master.Currencies, Master.Languages, Master.TimeZones, Master.TenantUsers';
PRINT '✓ Ready for seed data and stored procedures';
GO
